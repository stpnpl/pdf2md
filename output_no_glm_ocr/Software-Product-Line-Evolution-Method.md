...
![Table 3 diagram with blue arrows showing kaizen scenarios (1), (2), (3) among core asset types](Software-Product-Line-Evolution-Method/assets/page-004-img-01.png)
...
&\textit{(Here, } \sum \textit{ means sum of total number of products)} \\
C_{old\_way} &: \textit{Costs of developing a product before Kaizen} \\
C_{new\_way} &: \textit{Costs of developing a product after Kaizen} \\
C_{del} &: \textit{Costs of removing core assets} \\
C_{ref} &: \textit{Costs of refactoring core assets}
\end{aligned}
$$

![Figure 1. Kaizen Processes — PDCA cycle diagram with Plan, Do, Check, Act around core asset kaizen, standard, and core assets](Software-Product-Line-Evolution-Method/assets/page-006-img-01.png)
...
```mermaid
xychart-beta
    title "s12 vs s13 across categories"
    x-axis ["05a", "05b", "06a", "06b", "07a", "07b"]
    y-axis "Value" -8.00 --> 4.00
    line "s12:Detoxing-->Refactoring" [-4.00, -6.50, -5.00, -2.75, -0.50, 1.75]
    line "s13:Refactoring-->Detoxing" [-3.00, -5.00, -3.40, -1.00, 1.40, 3.80]
```

*A line chart comparing two series, s12:Detoxing-->Refactoring and s13:Refactoring-->Detoxing, across categories 05a–07b, with an embedded data table of values.*

<details>
<summary>Original figure</summary>

![Figure 2. Cumulative Effects for Two Scenarios — line chart comparing s12:Detoxing-->Refactoring and s13:Refactoring-->Detox…](Software-Product-Line-Evolution-Method/assets/page-006-img-02.png)
...
Figure 2. Cumulative Effects for Two Scenarios — line chart comparing s12:Detoxing-->Refactoring and s13:Refactoring-->Detoxing from 05a to 07b
...
&\textit{(Here, } n_1 \textit{ is the number of products in a specific product line.)} \\
C_{evo} &: \textit{Cost of evolving one product the old way} \\
C_{org} &: \textit{Cost of changing the organization to adopt} \\
&\quad \textit{software product line engineering} \\
C_{cab} &: \textit{Cost of core asset base} \\
C_{unique} &: \textit{Cost of building each product's unique part} \\
C_{reuse} &: \textit{Cost of using the core assets to build a product}
\end{aligned}
$$

The above equation assumes that an organization will develop new core assets. Our method considers $C_{del}$ and $C_{ref}$ in addition to $C_{cab}$. These costs are indispensable when the organization maintains the product line for a long time and changes it with time.
