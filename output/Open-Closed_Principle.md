...
*Figure 1 (Closed Client): a simple UML diagram where a concrete Client class uses a concrete Server class.*

<details>
<summary>Original figure</summary>

![Booch class diagram: dashed cloud shapes labeled Client and Server connected by a line with a circle at the Client end](Open-Closed_Principle/assets/page-002-img-01.png)
...
flowchart LR
    A["Client"]:::blob
    B["Abstract Server"]:::blob
    C["Server"]:::blob
    A --o B
    C --> B
    classDef blob stroke-dasharray:5 5,fill:none,stroke:#333
```

*Figure 2 'Open Client': a UML-style component diagram showing Client depending on Abstract Server (interface A), with Server realizing/inheriting from Abstract Server.*

<details>
<summary>Original figure</summary>

![Booch diagram: Client class connected to abstract AbstractServer class, with Server class deriving from AbstractServer](Open-Closed_Principle/assets/page-003-img-01.png)

*Figure 2 — Open Client*
...
We know that the type or meaning of this variable is never going to change. So why not make it `public` and let client code simply examine its contents? If this variable really never changes, and if all other clients obey the rules and only query the contents of `status`, then the fact that the variable to public does no harm at all. However, consider what happens if even one client takes advantage of the writable nature of `status`, and changes its value. Suddenly, this one client could affect every other client of `Device`. This means that it is impossible to close any client of `Device` against changes to this one misbehaving module. This is probably far too big a risk to take.
...
![Boxed code listing: Listing 9, RTTI violating the open-closed principle](Open-Closed_Principle/assets/page-012-img-01.png)

*Listing 9*

![Boxed code listing: Listing 10, RTTI that does not violate the open-closed principle](Open-Closed_Principle/assets/page-012-img-02.png)
...
```mermaid
classDiagram
    A *-- B : Contains - by value
    class A {
        -B itsB
    }
    note for A "Contains - by value: A and B have identical lifetimes. When A is destroyed, B will be destroyed too."
```

*Booch-notation diagram showing class A containing class B by value (identical lifetimes, filled circle at A joined by a line to a filled square at B).*

<details>
<summary>Original figure</summary>

![Booch notation: A contains B by value, filled diamond and filled square](Open-Closed_Principle/assets/page-014-img-01.png)
...
![Booch notation: A uses B, circle and line](Open-Closed_Principle/assets/page-014-img-02.png)
...
![Booch notation: A contains B by reference, filled diamond and unfilled square](Open-Closed_Principle/assets/page-014-img-03.png)
...
*Booch-style cloud notation showing class D inheriting (arrow) from class B, i.e., B is a public base class of D.*

<details>
<summary>Original figure</summary>

![Booch notation: inheritance, D inherits from B with arrow](Open-Closed_Principle/assets/page-014-img-04.png)
...
![Booch notation: invoking a member function, A to B with P, M, V annotations](Open-Closed_Principle/assets/page-014-img-05.png)
...
![Booch notation: using a container class Set<B*>, with 0..n multiplicity](Open-Closed_Principle/assets/page-014-img-06.png)
