...
```mermaid
flowchart LR
  D0[/"Feature<br/>Test case"/] -.-> S1(["1. Specify prompt"])
  P1["Prompt"] -.-> S1
  S1 --> S2(["2. Develop failing test"])
  D1[/"Test<br/>Prompt"/] -.-> S2
  S2 --> S3(["3. Develop code"])
  S3 --> S4(["4. Execute tests"])
  S4 --> DEC{"Pass/fail?"}
  DEC -- "Fail" --> S5(["5. Choose prompt"])
  S5 --> S3
  D2[/"Error Trace<br/>Code<br/>Test<br/>Prompt"/] -.-> S5
```

*Fig. 1 'Fully-automated pattern': a five-step TDD workflow where a developer specifies a prompt, an AI (ChatGPT icons) develops a failing test and code, tests are executed, and on failure a prompt is chosen to repeat code development, with prompt/document artifacts (red text = developer-modifiable parts) attached to the steps.*

<details>
<summary>Original figure</summary>

![Flowchart of the fully-automated pattern workflow](Generative_AI_for_Test_Driven_Development_-_arxiv/assets/page-004-img-01.png)
