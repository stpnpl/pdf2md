...
```mermaid
flowchart LR
    F[Feature Test case] -.-> A
    P0[/Prompt/] -.-> A
    A(["1 Specify prompt"]) --> B(["2 Develop failing test"])
    T1[/Test/] -.-> B
    P1[/Prompt/] -.-> B
    B --> C(["3 Develop code"])
    C --> D(["4 Execute tests"])
...
*A numbered flowchart of a fully-automated TDD pattern: feature/test-case input feeds prompt specification, failing test development, code development, test execution, and a pass/fail decision with a choose-prompt feedback loop.*
