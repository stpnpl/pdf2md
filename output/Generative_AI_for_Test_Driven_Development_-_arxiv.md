...
- First iteration: Use the Assertion First pattern in TDD and stubs and drivers to develop the first barely minimal test and production code for the feature ⟨*feature description*⟩ with input ⟨*names*⟩ and ⟨*values*⟩ and expected output ⟨*values*⟩
...
```mermaid
flowchart LR
    D1[("Feature<br/>Test case")]
    S1([1. Specify prompt])
    S2([2. Develop failing test])
    S3([3. Develop code])
    S4([4. Execute tests])
    D{Pass/fail?}
    S5([5. Choose prompt])
    P1[/Prompt/]
    P2[("Test<br/>Prompt")]
    P3[("Error Trace<br/>Code<br/>Test<br/>Prompt")]
...
*A five-step TDD-with-LLM process flowchart: specify prompt, develop failing test, develop code, execute tests, with a pass/fail decision looping back through 'Choose prompt' on fail.*

<details>
<summary>Original figure</summary>

![Workflow diagram of the fully-automated TDD pattern with five activities: Specify prompt, Develop failing test, Develop code…](Generative_AI_for_Test_Driven_Development_-_arxiv/assets/page-004-img-01.png)
...
Workflow diagram of the fully-automated TDD pattern with five activities: Specify prompt, Develop failing test, Develop code, Execute tests, Choose prompt, with Pass/Fail decision and Repeat loop
...
2. In the `test_TwoWordSpreadOdd` test, the `TextFormatter` variable was mistakenly assigned instead of `textFormatter`.
1. In the `test_WordCenterOdd` test, the expected result was corrected by removing the extra spaces.
...
We thank the practitioners who participated in the study for their valuable
contribution. Moritz Mock is partially funded by the National Recovery and
Resilience Plan (Piano Nazionale di Ripresa e Resilienza, PNRR - DM 117/2023).
