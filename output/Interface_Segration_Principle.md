...
- [But it's just a recompile.](#but-its-just-a-recompile)
...
classDiagram
    ...
    BaseClass <|-- Derived1
    BaseClass <|-- Derived2
    BaseClass o-- HadByReference : Had by Reference
    BaseClass *-- HadByValue : Had By Value
    BaseClass ..> Used
    note for BaseClass "Base Class"
    note for Derived1 "Derived 1"
    note for Derived2 "Derived 2"
    note for Used "Used"
    note for HadByReference "Had by Reference"
    note for HadByValue "Had By Value"
```

*A UML notation legend showing a Base Class inheriting to Derived 1 and Derived 2 (hollow triangle), with aggregation/computation diamonds linking to 'Had by Reference' and 'Had By Value', and a dashed dependency arrow to 'Used'.*

<details>
<summary>Original figure</summary>

![Sidebar: Unified Notation 0.8 — UML class diagram showing Base Class with Used dependency, Had by Reference aggregation, Had…](Interface_Segration_Principle/assets/page-001-img-01.png)
...
Sidebar: Unified Notation 0.8 — UML class diagram showing Base Class with Used dependency, Had by Reference aggregation, Had By Value composition, and Derived 1 / Derived 2 subclasses
...
In my last column (May 96) I discussed the principle of Dependency Inversion (DIP). This principle states that modules that encapsulate high level policy should not depend upon modules that implement details. Rather, both kinds of modules should depend upon abstractions. To be more succinct and simplistic, abstract classes should not depend upon concrete classes; concrete classes should depend upon abstract classes. A good example of this principle is the TEMPLATE METHOD pattern from the GOF¹ book. In this pattern, a high level algorithm is encoded in an abstract base class and makes use of pure virtual functions to implement its details. Derived classes implement those detailed virtual func-tions. Thus, the class containing the details depend upon the class containing the abstrac-tion.

In this article we will examine yet another structural principle: the Interface Segrega-tion Principle (ISP). This principle deals with the disadvantages of "fat" interfaces. Classes that have "fat" interfaces are classes whose interfaces are not cohesive. In other
...
![UML diagram: TimerClient at top, Door inherits from TimerClient, TimedDoor inherits from Door](Interface_Segration_Principle/assets/page-003-img-01.png)
...
classDiagram
    ...
    Abstract1 <|-- TimedDoor
    Abstract2 <|-- DoorTimerAdapter
    DoorTimerAdapter o-- TimedDoor
    TimedDoor ..> DoorTimerAdapter
```

*UML class diagram fragment showing TimedDoor and DoorTimerAdapter each inheriting (hollow-triangle arrows) from unnamed abstract parent classes (labels cropped, only \<<Abstract>> stereotypes visible), with an aggregation from DoorTimerAdapter to TimedDoor and a dashed dependency from TimedDoor to DoorTimerAdapter.*

<details>
<summary>Original figure</summary>

![UML diagram: TimedDoor and DoorTimerAdapter inherit from Door and TimerClient respectively; DoorTimerAdapter delegates to Ti…](Interface_Segration_Principle/assets/page-006-img-01.png)
...
UML diagram: TimedDoor and DoorTimerAdapter inherit from Door and TimerClient respectively; DoorTimerAdapter delegates to TimedDoor
...
![UML diagram: TimedDoor inherits from Door and TimerClient](Interface_Segration_Principle/assets/page-007-img-03.png)
...
*UML class diagram showing abstract class 'ATM UI' generalized by three concrete subclasses: Braille UI, Screen UI, and Speech UI.*

<details>
<summary>Original figure</summary>

![UML diagram: ATM UI (Abstract) inherited by Braille UI, Screen UI, Speech UI](Interface_Segration_Principle/assets/page-008-img-01.png)
...
*UML class diagram showing abstract Transaction superclass with three subclasses (Deposit, Withdrawl, Transfer Transaction), each depending on an abstract UI class (Figure 6, Segregated ATM UI Interface).*

<details>
<summary>Original figure</summary>

![UML diagram: abstract Transaction class with three derivatives — Deposit Transaction, Withdrawl Transaction, Transfer Transa…](Interface_Segration_Principle/assets/page-009-img-01.png)
...
UML diagram: abstract Transaction class with three derivatives — Deposit Transaction, Withdrawl Transaction, Transfer Transaction — each with a dashed dependency arrow converging to the abstract UI class
...
*UML class diagram fragment showing abstract classes Deposit UI, Withdrawl UI, and Transfer UI inheriting from abstract UI (with unnamed dependency boxes cropped at the top).*

<details>
<summary>Original figure</summary>

![UML diagram: Deposit Transaction, Withdrawl Transaction and Transfer Transaction each depend via dashed arrows on Deposit UI…](Interface_Segration_Principle/assets/page-009-img-02.png)
...
UML diagram: Deposit Transaction, Withdrawl Transaction and Transfer Transaction each depend via dashed arrows on Deposit UI, Withdrawl UI and Transfer UI abstract classes respectively, which all inherit into the abstract UI class
